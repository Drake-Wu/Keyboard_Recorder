	...
	case WM_INPUT:
		{
			...
			{
				if((GetKeyState(VK_CAPITAL) & 0x1) ^ shift_status)
					printf("%c", input.data.keyboard.VKey);
				else
			...
			else if (input.data.keyboard.VKey == VK_LSHIFT ||
				input.data.keyboard.VKey == VK_RSHIFT ||
				input.data.keyboard.VKey == VK_SHIFT)
			{
